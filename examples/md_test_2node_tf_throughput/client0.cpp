...
//    }
	...
	perf("Append into remote builder");
  ARROW_ASSIGN_OR_RAISE(std::shared_ptr<arrow::Array> remote_days, uint16builderRemote->Finish());
	...
	double t = perf("Writing to remote RAW: uint16_t, parallel, simd, unroll partial(64)");
	...
	t = perf("Writing years to remote RAW: int64_t, parallel, simd, unroll partial(64)");
	std::cout << "Comes to throughput of: " << ((uint64_t)SIZE*2UL / (1024UL*1024UL*1024UL) / (t/1000)) << " GB/s" << std::endl;
