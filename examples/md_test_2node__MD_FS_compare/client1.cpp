...
arrow::Status RunMain(int argc, char** argv) {
  ...
  schema = arrow::schema({field_day, field_month, field_year});
	...
	const int ITER = 20;
	const int SLICE_SIZE = SIZE / ITER;
	float md_measure[ITER][3]; // i, rows, time

	double start, end;
	unsigned long int sent_rows = 0;
	for (int i = 0; i < ITER; i++) {
		std::shared_ptr<arrow::RecordBatch> rbatch_slice = rbatch->Slice(0, (i+1)*SLICE_SIZE);
		start = perf(NULL);
		...
		md_measure[i][2] = end-start;
	}
	FloatArrayToCsv((float *)(&md_measure), ITER, 3, "./MD.csv");
	...
	sent_rows = 0;
	for (int i = 0; i < ITER; i++) {
		std::shared_ptr<arrow::Table> table_slice = table2->Slice(0, (i+1)*SLICE_SIZE);
		start = perf(NULL);
	...
	FloatArrayToCsv((float *)(&md_measure), ITER, 3, "./Full.csv");
	// for (int i = 0; i < ITER; i++) {
	// 	ARROW_RETURN_NOT_OK(orc.SendTableFull(table2, i*SIZE));
	// }
	perf("SendTableFull");
	...
	// Retrieve the "RemoteMemoryPool" instance
	...
	// Create a buiulder with the remote memory pool as allocator
