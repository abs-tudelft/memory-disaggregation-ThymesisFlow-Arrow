...
#define ARROW_THYMESISMALLOC // FIXME
...
#ifdef ARROW_THYMESISMALLOC
class ARROW_EXPORT ThymesismallocRemoteMemoryPool
    : public MemoryPool {
 public:
  explicit ThymesismallocRemoteMemoryPool(std::shared_ptr<OrchestratorClient> client, uint32_t id) 
    : client(client), id(id) {};

  std::string backend_name() const override { return "thymesismalloc_remote"; }
 ...
 private:
  ...
  // The regions "this" node has allocated remotely. 
  // These are the regions "this" device is responsible for free'ing
  std::vector<uintptr_t *> malloc_regions;
...
ARROW_EXPORT Status thymesismalloc_memory_pool(MemoryPool** out);
