...
#define TMALLOC_DEBUG_ENABLE

#ifdef TMALLOC_DEBUG_ENABLE
#define TMALLOC_DEBUG(...) printf(__VA_ARGS__)
#else
#define TMALLOC_DEBUG(...) do { } while(0)
#endif
...
{
	TMALLOC_DEBUG("tmalloc: size=%lu alignment=%lu\n", size, alignment);
    ...
    // If no suitable space, ran out of memory
    if (!ptr) {
			printf("ENOMEM\n");
			return NULL;
		}
...
{
	TMALLOC_DEBUG("tmalloc_addblock: addr=%p size=%lu\n", addr, size);
