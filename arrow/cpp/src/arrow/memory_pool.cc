...
enum class MemoryPoolBackend : uint8_t { System, Jemalloc, Mimalloc, Thymesismalloc };
...
#if defined(ARROW_THYMESISMALLOC)
    {"thymesismalloc", MemoryPoolBackend::Thymesismalloc},
#endif
...
class ThymesismallocMemoryPool
    : public BaseMemoryPoolImpl<memory_pool::internal::ThymesismallocAllocator> {
 public:
  std::string backend_name() const override { return "thymesismalloc"; }
  ...
  Status Allocate(int64_t size, uint8_t** out) override {
    ...
    return BaseMemoryPoolImpl::Allocate(size, out);
  }

  Status Reallocate(int64_t old_size, int64_t new_size, uint8_t** ptr) override {

    return BaseMemoryPoolImpl::Reallocate(old_size, new_size, ptr);
  }

  void Free(uint8_t* buffer, int64_t size) override {
    BaseMemoryPoolImpl::Free(buffer, size);
  }
...
class ThymesismallocDebugMemoryPool
    : public BaseMemoryPoolImpl<
          DebugAllocator<memory_pool::internal::ThymesismallocAllocator>> {
 public:
  std::string backend_name() const override { return "thymesismalloc"; }
};


Status ThymesismallocRemoteMemoryPool::Allocate(int64_t size, uint8_t** out) {
  std::cout << "ThymesismallocRemoteMemoryPool::Allocate id=" << id << std::endl;
  uintptr_t* out_p;
  // Proxy request to remote.
  ARROW_ASSIGN_OR_RAISE(out_p, client->Malloc((uint32_t)size));
  if (out_p == nullptr) {
    return Status::Invalid("Received unknown address");
  }
  // TODO check if returned address lies within the client's region

  *out = (uint8_t*)out_p;

  // Keep track of which regions I have allocated. If the object goes out of scope, only the regions allocated by me will be called free upon
  malloc_regions.push_back(out_p);

  return Status::OK();
}

Status ThymesismallocRemoteMemoryPool::Reallocate(int64_t old_size, int64_t new_size, uint8_t** ptr) {
  uintptr_t* out_p;
  // Proxy request to remote.
  ARROW_ASSIGN_OR_RAISE(out_p, client->Reallocate((uint32_t)old_size, (uint32_t)new_size, *ptr));
  if (out_p == nullptr) {
    return Status::Invalid("Received unknown address");
  }
  // TODO check if returned address lies within the client's region

  // Remove old pointer from malloc_regions
  for (std::vector<uintptr_t *>::iterator it = malloc_regions.begin(); it != malloc_regions.end();) {
    if (*it == (uintptr_t *)(*ptr)) {
      it = malloc_regions.erase(it);
    } else {
      ++it;
    }
  }
...
void ThymesismallocRemoteMemoryPool::Free(uint8_t* buffer, int64_t size) {
  // check if this address was malloc'ed by me
  for (std::vector<uintptr_t *>::iterator it = malloc_regions.begin(); it != malloc_regions.end();) {
    // If it is, free the memory and remove it from the list of regions I have allocated
    if (*it == (uintptr_t *)buffer) {
      printf("ThymesismallocRemoteMemoryPool::Free id=%d p=%p size=%ld\n", id, buffer, size);
      it = malloc_regions.erase(it);
      ARROW_UNUSED(client->Free((uintptr_t *)buffer, (uint32_t)size));
    } else {
      ++it;
    }
  }
...
#endif
#ifdef ARROW_THYMESISMALLOC
    case MemoryPoolBackend::Thymesismalloc:
      return IsDebugEnabled() ? std::unique_ptr<MemoryPool>(new ThymesismallocDebugMemoryPool)
                              : std::unique_ptr<MemoryPool>(new ThymesismallocMemoryPool);
#endif
    default:
...
#ifdef ARROW_THYMESISMALLOC
  MemoryPool* thymesismalloc_memory_pool() {
    if (IsDebugEnabled()) {
      return &thymesismalloc_debug_pool_;
    } else {
      return &thymesismalloc_pool_;
    }
  }
#endif
...
#endif
#ifdef ARROW_THYMESISMALLOC
  ThymesismallocMemoryPool thymesismalloc_pool_;
  ThymesismallocDebugMemoryPool thymesismalloc_debug_pool_;
#endif
#ifdef ARROW_MIMALLOC
...
Status thymesismalloc_memory_pool(MemoryPool** out) {
#ifdef ARROW_THYMESISMALLOC
  *out = global_state.thymesismalloc_memory_pool();
  return Status::OK();
#else
  return Status::NotImplemented("This Arrow build does not enable thymesisflow allocator");
#endif
}

// Status thymesis_add_mapping_pcie(tf_map region) {
// #ifdef ARROW_THYMESISMALLOC
//   return arrow::memory_pool::internal::ThymesismallocAllocator::AddMapping(region);
// #else
//   return Status::NotImplemented("This Arrow build does not enable ThymesisMalloc");
// #endif
// }

// void thymesis_print_maps() {
// #ifdef ARROW_THYMESISMALLOC
//   arrow::memory_pool::internal::ThymesismallocAllocator::print_loaded_config();
// #else
//   printf("This Arrow build does not enable ThymesisMalloc\n");
// #endif
// }
...
#ifdef ARROW_THYMESISMALLOC
    case MemoryPoolBackend::Thymesismalloc:
      return global_state.thymesismalloc_memory_pool();
#endif
#ifdef ARROW_MIMALLOC
    case MemoryPoolBackend::Mimalloc:
